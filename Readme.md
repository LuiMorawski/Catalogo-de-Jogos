...
- [x] **Avaliação única por autor** — um mesmo autor não pode avaliar o mesmo jogo mais de uma vez (`Jogo.AdicionarAvaliacao`)
- [x] **Reaproveitamento de gênero** — gêneros com o mesmo nome (ignorando maiúsculas/minúsculas) não são duplicados (`CatalogoServices.ObterOuCriarGenero`)
- [x] **Cálculo de ranking** — os jogos são ordenados pela média das notas recebidas (`Jogo.CalcularMediaAvaliacoes` + `CatalogoServices.ExibirRanking`)
